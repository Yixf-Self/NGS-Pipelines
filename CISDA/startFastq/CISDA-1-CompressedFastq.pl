...
my $HELP_g = '
        ...
        Step 1: Decompress all compressed FASTQ files and merge the two lanes of the same sample, 
                and quality statistics by using FastQC, NGS_QC_Toolkit and FASTX-Toolkit.
  
        Usage: 
               perl  CISDA-1-CompressedFastq.pl    [-v]    [-h]    [-i inputDir]    [-o outDir]  

        For instance: 
                     perl  CISDA-1-CompressedFastq.pl    -i 1-CompressedFastq          -o 2-FASTQ           
                     perl  CISDA-1-CompressedFastq.pl    --input 1-CompressedFastq     --output 2-FASTQ    
                     perl  CISDA-1-CompressedFastq.pl    --input 1-CompressedFastq     --output 2-FASTQ    >> 1-runLog.txt  2>&1
        ...
        -i inputDir,  --input inputDir        inputDir is the name of input folder that contains your Compressed Fastq files,
                                              the suffix of the Compressed Fastq files must be ".fastq.bz2".    (no default)
...
my $input_g  = '1-CompressedFastq';     ## This is only an initialization  value or suggesting value, not default value.
...
if($boole_g == 1) {
    ...
    print '    Please see help message by using "perl  CISDA-1-CompressedFastq.pl  -h" ';
    print "\n\n";
    exit 0;
}
...
print "\n\n        Decompressing the compressed fastq files and detecting the quality of FASTQ files by using FastQC......";
...
while ( my $file = readdir($DH_input) ) {     
        next unless $file =~ m/\.fastq\.bz2$/;
        next unless $file !~ m/^[.]/;
        next unless $file !~ m/[~]$/;
        my $temp = $file; 
        $temp =~ m/^(\w+_\w+_\w+_\w+_Rep[1-9]_?[1-2]?_Lane[1-2]\.fastq)\.bz2$/   or  die;
        my  $tempFastq = $1;
        system("bzip2   -cd    $input_g/$temp  > $input_g/$tempFastq");
        system( "fastqc    --outdir $FastQCdir1   --threads 10    --kmers 7     $input_g/$tempFastq       >> $FastQCdir1/$tempFastq.runLog   2>&1" );  
}




print "\n\n        Merging the two lanes of the same sample......";
...
for (my $i=0; $i<=$#twoLanes; $i++) {
    ...
    if( $twoLanes[$i] =~ m/^(\S+)_Lane1.fastq$/ ) {
        my $temp = $1;   
        $temp =~ m/^(\w+)_(\w+)_(\w+)_(\w+)_(Rep[1-9])_?[1-2]?$/   or  die;
...
for ( my $i=0; $i<=$#outputFiles_sort; $i++ ) {     
    ...
    my $numLines = @lines;
    $numLines = $numLines/4;
    $numreads[$#numreads+1] = "$temp\t$numLines";
}
open(NumReads_FH,  ">", "$output_g/2-NumberOfReads.xlsx" )  or  die;
print  NumReads_FH    "Samples\t#Reads\n";
for(my $i=0; $i<=$#numreads; $i=$i+1) {
