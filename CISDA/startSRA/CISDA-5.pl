...
my $HELP_g = '
        ...
        Step 5: Only  retain unique mapped, mismatch<=2 and MAPQ>=10 reads.  
        ...
        For instance: 
                     perl  CISDA-5.pl    -i 5-SortMapped          -o 6-FinalBAM           
                     perl  CISDA-5.pl    --input 5-SortMapped     --output 6-FinalBAM    
                     perl  CISDA-5.pl    --input 5-SortMapped     --output 6-FinalBAM    >>  5-runLog.txt  2>&1      
...
my $output_g = '6-FinalBAM';        ## This is only an initialization  value or suggesting value, not default value.
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    $temp =~ s/\.bam$//  or  die;
    system(`samtools  view   -hb   -q 10  -o $outDir1/$temp.q.bam       $inputDir1/$temp.bam                               >>$outDir1/$temp.runLog    2>&1`);    ## must use `,  cann't use "
    system("samtools  sort   -O bam       -o $outDir1/$temp.bam    -T $outDir1/$inputFiles[$i]      $outDir1/$temp.q.bam   >>$outDir1/$temp.runLog    2>&1");
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    my $temp = $inputFiles[$i];
    $temp =~ s/\.bam$//  or  die;
    ## XT Type: Unique/Repeat/N/Mate-sw             ## XM, Number of mismatches in the alignment
    system(`samtools  view   -h   -q 10   $inputDir2/$temp.bam      |   grep -P "(XT:A:U)|(^@)"  |   grep -P  "(XM:i:[0-2]|^@)"    >$outDir2/$temp.sam`);   ## must use `,  cann't use "
    system("samtools  sort   -O bam       -o $outDir2/$temp.bam    -T $outDir2/$inputFiles[$i]      $outDir2/$temp.sam     >>$outDir2/$temp.runLog    2>&1");
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    $temp =~ s/\.bam$//  or  die;
    ## NM  Edit distance
    system(`samtools  view   -h   -q 10  $inputDir3/$temp.bam     |   grep -P  "(NM:i:[0-2]|^@)"    >$outDir3/$temp.sam`);     ## must use `,  cann't use "
    system("samtools  sort   -O bam       -o $outDir3/$temp.bam    -T $outDir3/$inputFiles[$i]      $outDir3/$temp.sam     >>$outDir3/$temp.runLog    2>&1");
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    my $temp = $inputFiles[$i];
    $temp =~ s/\.bam$//  or  die;
    ## XA:i:<N>     Aligned read belongs to stratum <N>.  In the -n alignment mode, an alignment's "stratum" is defined as the number of mismatches in the "seed" region
    ## NM:i:<N>     Aligned read has an edit distance of <N>.
    system(`samtools  view   -h   -q 10  $inputDir4/$temp.bam   |   grep -P  "(XA:i:[0-2])|(^@)"    |   grep -P  "(NM:i:[0-2]|^@)"      >$outDir4/$temp.sam`);      ## must use `,  cann't use "
    system("samtools  sort   -O bam       -o $outDir4/$temp.bam    -T $outDir4/$inputFiles[$i]      $outDir4/$temp.sam     >>$outDir4/$temp.runLog    2>&1");
...
for(my $i=0; $i<=$#inputFiles; $i++) {
    ...
    my $temp = $inputFiles[$i];
    $temp =~ s/\.bam$//  or  die;
    ## XS:i:<N>   Only present if the SAM record is for an aligned read and more than one alignment was found for the read.
    ## XM:i:<N>	  The number of mismatches in the alignment. Only present if SAM record is for an aligned read. 
    system(`samtools  view   -h   -q 10   $inputDir5/$temp.bam   |   grep -v  "XS:i:"   |   grep -P  "(XM:i:[0-2]|^@)"   >$outDir5/$temp.sam`);     ## must use `,  cann't use "
    system("samtools  sort   -O bam       -o $outDir5/$temp.bam    -T $outDir5/$inputFiles[$i]      $outDir5/$temp.sam     >>$outDir5/$temp.runLog    2>&1");
