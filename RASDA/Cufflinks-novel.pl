...
my $outDir = "5-Cufflinks-haveNovel";
...
for(my $i=0; $i<=$#inNames1; $i++) {
    if($inNames1[$i] =~ m/\.bam$/) {
        my $temp = $inNames1[$i];
        $temp =~ s/\.bam$//  or  die;
        system("cufflinks    --output-dir $outDir1/$temp        --num-threads 6     --GTF-guide    0-Other/mm9/mm9-RefSeqGenes.GTF     --frag-bias-correct 0-Other/mm9/genome.fa     --multi-read-correct    --library-type fr-unstranded          --label $temp     $inDir1/$temp.bam     >>$outDir1/$temp.log.txt      2>&1"   );                                     
